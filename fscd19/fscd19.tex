...
becoming a neutral.  This mechanism provides us with unknowns of
...
are occupied by non-monadic values.
...
is to make explicit where the monad is envoked during monadic
...
extensionality ($\eta$) for function (an more generally, negative)
types.  Non-invertible eliminations are also happing in a chain,
...
In our presentation of the various lambda calculi and NbE, we use
presheaves for the abstract syntax and the semantics
\cite{altenkirchHofmannStreicher:ctcs95}.

In contrast to Zeilberger \cite{zeilberger:PhD}.
